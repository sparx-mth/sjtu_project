#!/usr/bin/env python3
"""
waypoint_follower.py  (v8-enhanced — detailed logging for mode & waypoint tracking)
...
class WaypointFollower:
    def __init__(self):
        ...
        rospy.loginfo("=" * 80)
        rospy.loginfo("waypoint_follower v8-enhanced ready  (X+YAW only, fixed altitude)")
        ...
        rospy.loginfo("  YAW_ALIGN: pos_radius=%.3f  yaw_radius=%.3f  "
                      "yaw_settle=%.3f  lead=%.1f%% (live: rosparam set ~yaw_lead_pct)",
                      self.pos_radius, self.yaw_radius, self.yaw_settle, self.yaw_lead_pct)
    ...
    def _demo_mode_cb(self, msg):
        ...
        if new_mode != self.current_demo_mode:
            rospy.loginfo("[MODE] DemoMode  %s → %s",
    ...
    def _request_demo_mode(self, mode):
        ...
        if self.requested_demo_mode != mode:
            ...
            self._last_request_pub_t = rospy.Time(0)
            intent = "🔄 YAW ALIGNMENT" if mode == DemoMode.TURNING else "➡️  FORWARD FLIGHT"
            rospy.loginfo("[MODE] INTENT: %s  (requesting %s, current=%s)",
                          intent, mode, self.current_demo_mode)
        ...
        if (self.request_timeout_sec > 0.0 and
            ...
            rospy.logwarn_throttle(2.0,
                                   "[MODE] DemoMode request '%s' not confirmed after %.1fs "
                                   "(current=%s) — holding zero velocity",
    ...
    def _path_cb(self, msg):
        ...
        if not pts:
            rospy.logwarn("waypoint_follower: empty path");
            return
        ...
        self.path_xy = pts
        self.wp_idx = 0
        rospy.loginfo("[PATH] NEW PATH  %d waypoints  "
    ...
    def _snapshot_yaw_lead(self):
        ...
        rospy.loginfo("[YAW_ALIGN] wp%d=(%.2f,%.2f)  cur=(%.2f,%.2f,yaw=%+.1f°)  "
                      "des_yaw=%+.1f°  sweep=%+.1f°  lead=%.1f%% (%.1f°)",
                      self.wp_idx + 1, tx, ty, cx, cy, math.degrees(yaw_cur),
                      math.degrees(yaw_des), math.degrees(eyaw), pct,
    ...
    def _ctrl_loop(self, _):
        ...
        if self.state == S.TAKING_OFF:
            ...
            if (self.cur_pose is not None
                    and self.cur_pose.position.z > self.takeoff_z_thresh):
                rospy.loginfo("[TAKEOFF] reached z=%.2f > %.2f — settling",
                              self.cur_pose.position.z, self.takeoff_z_thresh)
                self._enter(S.HOVER_SETTLE)
                return
            if (self.takeoff_count > 0 and
                    now - self.last_takeoff > rospy.Duration(
                        self.takeoff_timeout)):
                rospy.logwarn("[TAKEOFF] timeout after %d attempts — settling",
                              self.takeoff_count)
                self._enter(S.HOVER_SETTLE)
            return
        ...
        if self.state == S.YAW_ALIGN:
            # Handshake: stop, request TURNING, wait for confirm
            # before commanding any wz. The mode handshake ensures
            # safe transitions between X-control and YAW-control.
            if not self._ensure_mode(DemoMode.TURNING):
                return
            self._set_freeze(self.freeze_during_yaw)

            if not self.path_xy or self.wp_idx >= len(self.path_xy):
            ...
            yaw_des = math.atan2(ty - cy, tx - cx)
            eyaw = wrap_pi(yaw_des - yaw_cur)
            eyaw_lead = eyaw - math.copysign(self._yaw_align_lead, eyaw)

            if abs(eyaw_lead) <= self.yaw_settle:
                rospy.loginfo("[YAW_ALIGN] ✓ ACQUIRED  yaw_err=%+.2f° ≤ settle=%.2f°  "
                              "→ ADVANCE",
                              math.degrees(eyaw), math.degrees(self.yaw_settle))
                self._set_freeze(False)
                self._publish_zero()
                self._enter(S.ADVANCE)
                return
        ...
        if self.state == S.ADVANCE:
            ...
            if captured or passed:
                if passed and not captured:
                    rospy.loginfo("[WP] wp%d PASSED  d=%.2fm bearing=%+.0f° "
                                  "→ advancing to wp%d",
                                  self.wp_idx + 1, d,
                                  math.degrees(bearing_err),
                                  self.wp_idx + 2)
                else:
                    rospy.loginfo("[WP] wp%d ✓ ACQUIRED  d=%.3fm ≤ pos_radius=%.3f  "
                                  "→ advancing to wp%d",
                                  self.wp_idx + 1, d, self.pos_radius,
                                  self.wp_idx + 2)
                self.wp_idx += 1
                if self.wp_idx >= len(self.path_xy):
                    rospy.loginfo("[GOAL] ✓ GOAL REACHED — braking")
                    self._enter(S.BRAKE)
                    return
                ...
                if abs(next_err) < self.skip_yaw_thresh:
                    rospy.loginfo("[PATH] wp%d→%d GLIDE  next_yaw_err=%+.0f° ≤ "
                                  "skip_thresh=%.0f°",
                    ...
                    rospy.loginfo("[PATH] wp%d→%d CORNER  next_yaw_err=%+.0f° > "
                                  "skip_thresh=%.0f° → brake & re-align",
                                  self.wp_idx, self.wp_idx + 1,
                                  math.degrees(next_err),
                                  math.degrees(self.skip_yaw_thresh))
                    self._enter(S.BRAKE)
            ...
            if abs(actual_drift) > self.yaw_drift_thresh:
                rospy.loginfo("[DRIFT] yaw_drift=%+.0f° > %.0f° → brake & re-align",
        ...
        if self.state == S.BRAKE:
            ...
            if stopped or timed_out:
                if timed_out and not stopped:
                    rospy.logwarn("[BRAKE] timeout (vx=%.3f) — proceeding anyway",
                                  self.last_vx)
                if self.wp_idx >= len(self.path_xy):
                    self._enter(S.DONE)
                else:
    ...
    def _status(self, _):
        ...
        extra = ""
        mode_str = self.current_demo_mode if self.current_demo_mode else "none"
        if self.state == S.TAKING_OFF:
            ...
            extra = ("  wp=%d/%d target=(%.2f,%.2f) d=%.2fm yaw_err=%5.1f°  "
                     "mode=%s"
                     % (self.wp_idx + 1, len(self.path_xy), tx, ty, d, ey, mode_str))
