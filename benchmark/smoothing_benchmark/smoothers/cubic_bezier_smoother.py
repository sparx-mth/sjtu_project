#!/usr/bin/env python3
"""
Cubic Bezier (Hermite) Trajectory Smoother
------------------------------------------
Converts discrete waypoints into a smooth trajectory using cubic Hermite splines
...
class CubicBezierSmoother:
    ...
    at each waypoint, giving us control over the trajectory's orientation.
    """
    ...
    def _compute_tangents(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        ensuring smooth heading transitions.
        """
        n = len(self._xs)
        dx = np.zeros(n)
        dy = np.zeros(n)
...
def smooth_waypoints(
        ...
        tangent_scale: float = 0.5,
        cruise_speed: float = 0.4
) -> Optional[CubicBezierSmoother]:
    """Create smooth trajectory from waypoints. Returns None on failure."""
    try:
        return CubicBezierSmoother(waypoints_x, waypoints_y,
...
if __name__ == "__main__":
    ...
    if traj:
        print(f"Cubic Bezier (Hermite) Trajectory")
