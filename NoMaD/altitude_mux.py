...
class AltitudeMux(Node):
    ...
    def __init__(self):
        ...
        # ---------- Params ----------
        self.declare_parameter("z_ref", 1.1)
    ...
    def on_nav(self, msg: Twist):
        ...
        is_zero = (
                ...
                abs(msg.linear.z) < 1e-6 and
                abs(msg.angular.z) < 1e-6
        )

        # Explicit stop / hover command from teleop ('s')
        if is_zero:
            self.last_nav_cmd.linear.x = 0.0
            self.last_nav_cmd.linear.y = 0.0
            self.last_nav_cmd.angular.z = 0.0
            self.last_nav_cmd.linear.z = 0.0
            self.have_nav = True
            return

        # Latch planar motion if present
        if abs(msg.linear.x) > 1e-6:
            self.last_nav_cmd.linear.x = float(msg.linear.x)
            self.last_nav_cmd.linear.y = 0.0
            self.last_nav_cmd.angular.z = 0.0

        elif abs(msg.linear.y) > 1e-6:
            self.last_nav_cmd.linear.x = 0.0
            self.last_nav_cmd.linear.y = float(msg.linear.y)
            self.last_nav_cmd.angular.z = 0.0

        elif abs(msg.angular.z) > 1e-6:
            self.last_nav_cmd.linear.x = 0.0
            self.last_nav_cmd.linear.y = 0.0
            self.last_nav_cmd.angular.z = float(msg.angular.z)

        # z is only a temporary manual override
        self.last_nav_cmd.linear.z = float(msg.linear.z)
    ...
    def step(self):
        ...
        base = self.last_nav_cmd if self.have_nav else Twist()
        nav_ok = self.have_nav
