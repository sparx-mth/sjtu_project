...
# Standard rotation from link frame to optical frame: Rz(+90°) then Rx(-90°)
# Quaternion for that rotation is approximately (-0.5, -0.5, 0.5, 0.5)
#ros2 run tf2_ros static_transform_publisher \
 # 0 0 0  0.0 -0.70710678 0.0 0.70710678 \
#ros2 run tf2_ros static_transform_publisher \
...
case "$TAG_EXTRA_ROT" in
  none)   EX_QX=0; EX_QY=0; EX_QZ=0; EX_QW=1 ;;
  Rx90)   EX_QX=0.70710678; EX_QY=0; EX_QZ=0; EX_QW=0.70710678 ;;
  Rx-90)  EX_QX=-0.70710678; EX_QY=0; EX_QZ=0; EX_QW=0.70710678 ;;
  Ry90)   EX_QX=0; EX_QY=0.70710678; EX_QZ=0; EX_QW=0.70710678 ;;
  Ry-90)  EX_QX=0; EX_QY=-0.70710678; EX_QZ=0; EX_QW=0.70710678 ;;
  Rz90)   EX_QX=0; EX_QY=0; EX_QZ=0.70710678; EX_QW=0.70710678 ;;
  Rz-90)  EX_QX=0; EX_QY=0; EX_QZ=-0.70710678; EX_QW=0.70710678 ;;
  *)      EX_QX=0; EX_QY=0; EX_QZ=0; EX_QW=1 ;;
esac

# Publish an adjusted GT frame as a child of tag36h11:14_gt
TAG_GT_ALIGNED="${TAG_GT_FRAME}_aligned"
ros2 run tf2_ros static_transform_publisher \
  0 0 0  $EX_QX $EX_QY $EX_QZ $EX_QW \
  "$TAG_GT_FRAME" "$TAG_GT_ALIGNED" &
pids+=($!)

# Make the evaluator compare against the aligned frame
export TAG_GT_FRAME="$TAG_GT_ALIGNED"
...
export CAM_OPTICAL TAG_EST_FRAME TAG_GT_FRAME
python3 "$EVAL_PY" &
pids+=($!)
